...
public:
    ...
    // recorded draw.
    //
    // Called for BOTH the raster and path-tracing branches, so the frame slot
    // and the ring are advanced uniformly regardless of which one runs.
    ...
    // Did every cascade in the last shadow pass write the SAME number of object
    // records?
    //
    // All four cascades share object range [0, N) - BeginShadowCascade rewinds
    // the cursor rather than appending a private range per cascade, which is
    // what keeps object capacity at 2 x maxDraws and makes constant-ring
    // pressure flat in the entity count. That sharing is correct ONLY while
    // every cascade draws the same casters in the same order, so that draw i
    // means the same entity in all four and the byte-identical rewrites are
    // genuinely identical.
    //
    // Nothing in the type system enforces that. Per-cascade frustum culling is
    // the obvious future change that breaks it, and it would break it silently:
    // three cascades would rasterise the fourth's transforms and the shadows
    // would merely look wrong. A record-count mismatch is the cheapest
    // detectable symptom, so it is checked here and surfaced as a smoke marker.
    // It is a necessary condition, not a sufficient one - a reordering that
    // preserved the count would slip through - but it catches every filter
    // change, which is the realistic failure.
    bool ShadowCascadeRecordsUniform() const
    {
        for (uint32_t c = 1; c < m_cascadesBegunThisPass && c < core::kShadowCascadeCount; ++c)
            if (m_shadowCascadeRecords[c] != m_shadowCascadeRecords[0]) return false;
        return true;
    }
...
private:
    ...
    // Object records each cascade wrote, latched as each cascade ends. All four
    // must agree, because all four share object range [0, N) - see
    // ShadowCascadeRecordsUniform.
    uint32_t                     m_shadowCascadeRecords[core::kShadowCascadeCount] = {};
