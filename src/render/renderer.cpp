...
void Renderer::Shutdown()
{
    ...
    for (uint32_t i = 0; i < kFrameCount; i++)
    {
        ...
        if (m_lineVB[i] && m_lineVBMapped[i])
        {
            m_lineVB[i]->Unmap(0, nullptr);
...
bool Renderer::CreateRootSignature(ID3D12Device* device)
{
    ...
    if (featureData.HighestVersion >= D3D_ROOT_SIGNATURE_VERSION_1_1)
    {
        ...
        //
        // ALL, not VERTEX. The probe is written from three stages: both vertex
        // shaders write the object words, and basic_ps writes the material
    ...
    else
    {
        ...
        // ALL for the same reason as the v1.1 branch above: basic_ps writes the
        // material words. The two branches must stay identical.
        rootParams[7].ParameterType             = D3D12_ROOT_PARAMETER_TYPE_UAV;
        rootParams[7].Descriptor.ShaderRegister = 0;
        rootParams[7].Descriptor.RegisterSpace  = 4;
        rootParams[7].ShaderVisibility          = D3D12_SHADER_VISIBILITY_ALL;
...
// =============================================================================
// Line pipeline — colored world-space segments (orbit traces / map overlays)
// =============================================================================
namespace
{
...
bool Renderer::CreateLinePipeline(ID3D12Device* device)
{
    auto vs = CompileShaderFromFile(L"shaders/line_vs.hlsl", "main", "vs_5_1");
    auto ps = CompileShaderFromFile(L"shaders/line_ps.hlsl", "main", "ps_5_1");
    if (!vs || !ps)
    {
        core::Log::Error("Failed to compile line shaders");
        return false;
    }

    D3D12_ROOT_PARAMETER param = {};
    param.ParameterType             = D3D12_ROOT_PARAMETER_TYPE_CBV; // b0: viewProj
    ...
    if (FAILED(hr))
    {
        core::Log::Errorf("Line root signature serialize failed: 0x%08X", hr);
        return false;
    }
    hr = device->CreateRootSignature(0, sigBlob->GetBufferPointer(),
                                     sigBlob->GetBufferSize(),
                                     IID_PPV_ARGS(&m_lineRootSig));
    if (FAILED(hr))
    {
        core::Log::Errorf("Line CreateRootSignature failed: 0x%08X", hr);
        return false;
    }
    m_lineRootSig->SetName(L"LineRootSignature");

    D3D12_GRAPHICS_PIPELINE_STATE_DESC pso = {};
    pso.pRootSignature = m_lineRootSig.Get();
    pso.VS = { vs->GetBufferPointer(), vs->GetBufferSize() };
    pso.PS = { ps->GetBufferPointer(), ps->GetBufferSize() };
    pso.InputLayout = { kLineLayout, static_cast<UINT>(std::size(kLineLayout)) };

    pso.RasterizerState.FillMode        = D3D12_FILL_MODE_SOLID;
    pso.RasterizerState.CullMode        = D3D12_CULL_MODE_NONE; // lines have no facing
    pso.RasterizerState.DepthClipEnable = TRUE;

    // Alpha-blend into the HDR target; alpha rides the vertex color.
    ...
    // Reversed-Z depth TEST (so scene geometry occludes lines) but no depth WRITE.
    ...
    hr = device->CreateGraphicsPipelineState(&pso, IID_PPV_ARGS(&m_linePSO));
    if (FAILED(hr))
    {
        core::Log::Errorf("Line CreateGraphicsPipelineState failed: 0x%08X", hr);
        return false;
    }
    m_linePSO->SetName(L"LinePSO");
    core::Log::Info("Line pipeline created (orbit traces / map overlays)");
    return true;
}

bool Renderer::EnsureLineVertexBuffer(D3D12Device& device, uint32_t vertexCount)
{
    if (m_lineVB[0] && vertexCount <= m_lineVBCapacity)
        return true;

    uint32_t newCap = m_lineVBCapacity ? m_lineVBCapacity : 2048u;
    while (newCap < vertexCount) newCap *= 2u;

    const uint64_t byteSize = static_cast<uint64_t>(newCap) * sizeof(LineVertex);
    ...
    for (uint32_t i = 0; i < kFrameCount; ++i)
    {
        ...
        if (m_lineVB[i])
        {
            if (m_lineVBMapped[i]) { m_lineVB[i]->Unmap(0, nullptr); m_lineVBMapped[i] = nullptr; }
            device.DeferredRelease(m_lineVB[i]);
        }
        ...
        if (FAILED(hr))
        {
            core::Log::Errorf("Line vertex buffer create failed: 0x%08X", hr);
            return false;
        }
        D3D12_RANGE noRead = { 0, 0 };
        hr = buf->Map(0, &noRead, reinterpret_cast<void**>(&m_lineVBMapped[i]));
        if (FAILED(hr) || !m_lineVBMapped[i])
        {
            core::Log::Errorf("Line vertex buffer map failed: 0x%08X", hr);
            return false;
        }
        m_lineVB[i] = std::move(buf);
        wchar_t name[16];
        swprintf_s(name, L"LineVB[%u]", i);
        m_lineVB[i]->SetName(name);
    }
    m_lineVBCapacity = newCap;
    return true;
}

void Renderer::DrawLines(D3D12Device& device, const LineVertex* verts, uint32_t count,
                         const core::Mat4x4& viewProj)
{
    if (count == 0u || !verts || !m_linePSO)
        return;
    if (!EnsureLineVertexBuffer(device, count))
        return;

    std::memcpy(m_lineVBMapped[m_currentFrame], verts,
                static_cast<size_t>(count) * sizeof(LineVertex));

    const D3D12_GPU_VIRTUAL_ADDRESS cbVA =
        UploadCB(viewProj.Data(), sizeof(float) * 16u);
    if (cbVA == 0)
        return; // CB ring overflow (already logged)

    auto* cmd = device.CmdList();
    cmd->SetPipelineState(m_linePSO.Get());
    cmd->SetGraphicsRootSignature(m_lineRootSig.Get());
    cmd->SetGraphicsRootConstantBufferView(0, cbVA);

    D3D12_VERTEX_BUFFER_VIEW vbv = {};
    vbv.BufferLocation = m_lineVB[m_currentFrame]->GetGPUVirtualAddress();
    vbv.SizeInBytes    = count * sizeof(LineVertex);
                        ...
                        float emissiveStrength)
{
    ...
    // The self-identifying index, stamped at the slot being written, exactly as
    // WriteObjectRecord does for the object record. basic_ps writes it back and
    // ReadDrawProbe requires it to be the slot the CPU chose.
    material.recordId = materialIndex;
...
bool Renderer::ReadDrawProbe(DrawProbeValidation& validation)
{
    ...
    // Distinct-marker tallies. The markers are record indices + 1 drawn from a
    // known range, so a bitset over the record count is exact and needs no
    // sorting - and a marker outside the range is already a mismatch by the hash
    // and equality checks below.
    std::vector<bool> shadowSeen(m_drawProbeRecordCount + 1u, false);
    std::vector<bool> mainSeen(m_drawProbeRecordCount + 1u, false);
    std::vector<bool> materialSeen(m_drawProbeMaterialRecords + 1u, false);

    for (uint32_t i = 0; i < m_drawProbeRecordCount; ++i)
    {
        const bool isShadow = i < m_drawProbeShadowRecords;
        uint32_t& checked  = isShadow ? validation.shadowRecordsChecked
                                      : validation.mainRecordsChecked;
        uint32_t& mismatch = isShadow ? validation.shadowMismatches
                                      : validation.mainMismatches;

        ++checked;
        // Two independent claims about the same slot: the record's CONTENT
        // agrees byte for byte (hash), and the record's IDENTITY is the slot the
        // CPU allocated (marker, read out of the record rather than out of the
        // root constant that selected it).
        ...
        {
            auto& seen = isShadow ? shadowSeen : mainSeen;
            seen[actual[i].objectMarker - 1u] = true;
        }
        ...
        if (materialIndex >= m_drawProbeMaterialRecords) continue;

        // Both words zero means the pixel stage never ran for this draw - it
        // shaded no pixels. That is not a mismatch; it is an absence of
        // evidence, counted separately so it stays visible.
        if (actual[i].materialHash == 0 && actual[i].materialMarker == 0)
        {
        ...
        {
            materialSeen[actual[i].materialMarker - 1u] = true;
        }
    }

    auto countSet = [](const std::vector<bool>& bits) {
        uint32_t n = 0;
        for (bool b : bits) if (b) ++n;
        return n;
    };
    validation.shadowDistinctMarkers   = countSet(shadowSeen);
    validation.mainDistinctMarkers     = countSet(mainSeen);
    validation.materialDistinctMarkers = countSet(materialSeen);
