...
double SweptMinSeparation(const Vec3d& d0, const Vec3d& d1)
{
    ...
    return (d0 + b * t).Length();
}

// Classify the contact of pair (i,j) at a micro-step boundary. `pen` is set to
// contact - sep (may be < 0 for a graze that has separated by the boundary).
                     ...
                     const CloseEncounterConfig& cfg, double& pen)
{
    ...
    if (!(a.mu + b.mu > 0.0))       return kNone;   // no defined impulse / accretion
    ...
    const Vec3d  n  = d1.Normalized();                          // safe: sep > 0
    const double vn = (a.velocity - b.velocity).Dot(n);
    if (vn >= 0.0) return kNone;                                // separating/tangential: no impulse
    return kBounce;
}
...
{
    const Vec3d diff = a.position - b.position;
    const double sep = diff.Length();
    if (!(sep > 0.0)) return false;
    const Vec3d  n  = diff * (1.0 / sep);
    const double vn = (a.velocity - b.velocity).Dot(n);
                             ...
                             double dt, const CloseEncounterConfig& cfg)
{
    ...
    for (size_t i = 0; i < sz; ++i)
        for (size_t j = i + 1; j < sz; ++j)
        {
            ...
            const double closing = vrel.Length() * dt;
            ...
            const double nAcc = (Lacc > 0.0) ? std::ceil(closing / (cfg.eta * Lacc)) : 0.0;
                        ...
                        double dt, const CloseEncounterConfig& /*cfg*/)
{
    ...
    for (size_t i = 0; i < sz; ++i)
        for (size_t j = i + 1; j < sz; ++j)
        {
            ...
            const NBodyParticle& b = bodies[j];
            if (!a.isSource && !b.isSource) continue;
            const double sep     = (a.position - b.position).Length();
            const double closing = (a.velocity - b.velocity).Length() * ((dt > 0.0) ? dt : 0.0);
                               ...
                               CloseEncounterReport& report)
{
    ...
    if (n < 2 || prevPos.size() != bodies.size()) return;

    // 1. Classify every pair once against the input state.
    std::vector<uint32_t> parent(n);
    for (uint32_t k = 0; k < n; ++k) parent[k] = k;
    bool anyMerge = false;
    std::vector<double> compMaxPen(n, 0.0); // per-eventual-root; folded after union
    ...
    struct BounceEdge { uint64_t lo, hi; double pen; };
    ...
    for (uint32_t i = 0; i < n; ++i)
        ...
        {
            double pen = 0.0;
            const ContactKind kind = Classify(bodies, prevPos, i, j, cfg, pen);
            if (kind == kMerge)
            {
            ...
            else if (kind == kBounce)
            {
                ...
                bounceEdges.push_back({ std::min(a, b), std::max(a, b), pen });
            }
        }
    ...
    if (anyMerge)
    {
        ...
        out.reserve(n);
        std::vector<bool> rootEmitted(n, false);
    ...
    if (!bounceEdges.empty())
    {
        ...
        const int passes = (cfg.solverIterations > 0) ? cfg.solverIterations : 1;
        for (int pass = 0; pass < passes; ++pass)
            for (size_t e = 0; e < bounceEdges.size(); ++e)
            {
                ...
                if (ApplyBounce(bodies[ia->second], bodies[ib->second], cfg.restitution))
                    applied[e] = true;
            }
