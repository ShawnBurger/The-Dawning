...
LoadedModelResources UploadImportedModelResources(
    Scene& scene,
    ...
    const std::string& sourceLabel)
{
    ...
    const auto rollback = [&]()
    {
        for (auto it = registeredMeshes.rbegin(); it != registeredMeshes.rend(); ++it)
        {
            const MeshHandle handle = *it;
            resources.RemoveMesh(handle, device);
        }
        for (auto it = registeredTextures.rbegin(); it != registeredTextures.rend(); ++it)
        {
    ...
    try
    {
        ...
        registeredTextures.reserve(model.images.size());
        registeredMeshes.reserve(model.primitives.size());
        loaded.textures.reserve(model.images.size());
        loaded.primitives.reserve(model.primitives.size());
        loaded.uploadBuffers.reserve(model.images.size() + model.primitives.size() * 2u);
        ...
        for (size_t i = 0; i < model.images.size(); ++i)
        {
            ...
            render::Texture tex = render::CreateTexture2DFromWICMemory(d3dDevice,
                                                                       cmd,
                                                                       img.embeddedBytes.data(),
                                                                       img.embeddedBytes.size(),
                                                                       texUpload,
                                                                       L"ImportedImage");
            if (!tex.IsValid())
            {
                core::Log::Warnf("Model load: image %zu failed to decode; material will "
                                 "fall back to its scalar factor",
                                 i);
                continue;
            }
            ...
            const TextureHandle handle = resources.AddTexture(std::move(tex), texName.c_str());
            if (!handle.IsValid())
            {
            ...
            loaded.textures.push_back(handle);
        ...
        for (size_t p = 0; p < model.primitives.size(); ++p)
        {
            ...
            render::Mesh mesh = render::CreateMesh32(d3dDevice,
                                                     cmd,
                                                     vertices.data(),
            ...
            if (!mesh.IsValid())
            {
                ...
                core::Log::Errorf(
                    "Model load: CreateMesh32 failed for %s primitive %zu", sourceLabel.c_str(), p);
                rollback();
                return loaded;
            }
            ...
            {
                material =
                    ToEngineMaterial(model, model.materials[prim.materialIndex], imageHandles);
            }

            LoadedModelPrimitive loadedPrimitive;
            loadedPrimitive.sourcePrimitiveIndex = static_cast<uint32_t>(p);
            loadedPrimitive.mesh = meshHandle;
            loadedPrimitive.material = material;
            loadedPrimitive.name = meshName;
            loaded.primitives.push_back(std::move(loadedPrimitive));
        ...
        loaded.vertexCount = totalVertices;
        loaded.indexCount = totalIndices;
        loaded.decodedImageCount = decodedImages;
        loaded.ok = true;
        return loaded;
    }
    ...
    {
        rollback();
        loaded.error = "allocation failure while uploading model resources";
        return loaded;
    }
    catch (...)
    {
        rollback();
        loaded.error = "unexpected failure while uploading model resources";
        return loaded;
    }
}

LoadedModel InstantiateLoadedModel(Scene& scene,
                                   render::D3D12Device& device,
                                   render::Renderer& renderer,
...
{
    LoadedModel loaded;
    loaded.resources = std::move(resources);
    if (!loaded.resources.ok)
    {
        loaded.error = loaded.resources.error;
        return loaded;
    }

    const auto rollback = [&]() noexcept
    {
        for (auto it = loaded.entities.rbegin(); it != loaded.entities.rend(); ++it)
            scene.DestroyEntity(*it);
        loaded.entities.clear();
        ReleaseLoadedModelResources(loaded.resources, scene, device, renderer);
    };

    try
    {
        loaded.entities.reserve(loaded.resources.primitives.size());
        for (const LoadedModelPrimitive& primitive : loaded.resources.primitives)
        {
            const ecs::Entity entity = scene.CreateRenderable(
                primitive.name.c_str(), primitive.mesh, primitive.material, baseTransform);
            if (entity.IsNull())
            {
                loaded.error = "entity creation failed for a model primitive";
                rollback();
                return loaded;
            }
    ...
    {
        rollback();
        loaded.error = "allocation failure while instantiating model entities";
        return loaded;
    }
    catch (...)
    {
        rollback();
        loaded.error = "unexpected failure while instantiating model entities";
        return loaded;
    }
    ...
    core::Log::Infof("[SMOKE] model_loaded=ok model_source=%s model_primitives=%zu model_vertices=%llu model_indices=%llu model_images=%u",
                     ...
                     static_cast<unsigned long long>(loaded.resources.vertexCount),
                     static_cast<unsigned long long>(loaded.resources.indexCount),
                     loaded.resources.decodedImageCount);
    return loaded;
}
                               ...
                               const ecs::Transform& baseTransform)
{
    ...
    LoadedModelResources resources = UploadImportedModelResources(
        scene, device, renderer, imported.model, path.string());
    return InstantiateLoadedModel(
        scene, device, renderer, std::move(resources), "gltf", baseTransform);
}
    ...
    const std::filesystem::path& path)
{
    ...
    LoadedModelResources loaded = UploadImportedModelResources(
        scene, device, renderer, cooked.model, path.string());
    if (loaded.ok)
        loaded.sourceSha256 = cooked.metadata.sourceSha256;
    return loaded;
}
...
{
    ResourceManager& resources = scene.GetResources();
    for (auto it = loaded.primitives.rbegin(); it != loaded.primitives.rend(); ++it)
    {
        if (resources.IsValidMesh(it->mesh))
            resources.RemoveMesh(it->mesh, device);
    }
    for (auto it = loaded.textures.rbegin(); it != loaded.textures.rend(); ++it)
    {
        if (resources.IsValidTexture(*it))
            resources.RemoveTexture(*it, device, renderer);
    }
    loaded.primitives.clear();
    loaded.textures.clear();
    loaded.uploadBuffers.clear();
...
{
    LoadedModelResources resources =
        LoadCookedModelResources(scene, device, renderer, path);
    return InstantiateLoadedModel(
        scene, device, renderer, std::move(resources), "cooked", baseTransform);
}
