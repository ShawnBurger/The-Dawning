...
Verification includes a unit-test target covering core math and ECS invariants,
a Windows GitHub Actions build-and-test job, and an end-to-end render smoke test
(`tools/smoke_test.ps1`). Render and DXR behavior is still exercised locally,
not by CI. Read "DONE" below as "runs, and later layers depend on it", not as
"finished, hardened, or fully tested".
...
- Path tracing shader (path_trace.hlsl):
  ...
  - PCG hash RNG with each dimension hashed in sequence. It is seeded from a
    wall-clock dispatch counter, not the accumulation index, so moving-camera
    frames do not repeat one random sequence
  - Temporal accumulation using a progressive running mean weighted 1/(n+1).
    Camera or quality changes reset the mean; reprojection is not implemented
  - Firefly clamping against a fixed per-sample luminance ceiling. NaN and Inf
    samples are dropped before they can enter the history buffer
  - Indirect specular uses GGX VNDF importance sampling with a separable Smith G
    term. Rays sampled below the horizon terminate the path
...
Run render smoke tests serially within one checkout because they share one log:
...
Phase 1 (current): Path tracing with NEE, VNDF specular sampling, and a
         ...
         Reprojection is not implemented, so accumulation restarts whenever
         the view changes instead of reusing history
...
1. TARGET, NOT YET ENFORCED - double precision (Vec3d) for world positions,
   ...
   matches CONVERSATION_CONTEXT.md:72. Reality at this baseline: `Vec3d` has no
   references outside its own definition in `core/types.h`.
   `ecs::Transform::position` and `Camera::m_position` are both `Vec3f`, and
   `Vec3d` lacks operations needed for a direct conversion. Inspect current
   code before assuming the retrofit is complete.
